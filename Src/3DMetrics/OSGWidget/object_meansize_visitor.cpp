#include "object_meansize_visitor.h"
...
ObjectMeanSizeVisitor::ObjectMeanSizeVisitor() : osg::NodeVisitor(TRAVERSE_ALL_CHILDREN),
    m_count(0),
    m_totalZize(0)
{
}

ObjectMeanSizeVisitor::~ObjectMeanSizeVisitor()
{
}

void ObjectMeanSizeVisitor::apply ( osg::Node &node )
{
    traverse(node);
}

void ObjectMeanSizeVisitor::apply( osg::Geode &geode )
{
        ...
        if ( current_geometry )
        {
            ...
            for(unsigned int j = 0; j < primitive_list.size(); j++)
            {
                ...
                for(unsigned int k = 0; k < primitive_set->getNumIndices()-2; k=k+inc)
                {
                    ...
                    // use mean edge length o,stead of area
                    m_totalZize += (AB + BC + AC) / 3;
                    m_count++;
                }
