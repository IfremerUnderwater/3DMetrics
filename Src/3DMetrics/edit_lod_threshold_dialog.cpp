    ...
    ui(new Ui::EditLODThresholdDialog)
{
    ...
    QObject::connect(ui->okButton, SIGNAL(clicked()), this, SLOT(slot_ok()));
...
void EditLODThresholdDialog::slot_ok()
{
    if(m_threshold2 <= m_threshold1)
    {
        QMessageBox::warning( this, tr("LOD Thresholds"),tr("Threshold2 must be > Threshold1"));
        return;
    }
    accept();
}
